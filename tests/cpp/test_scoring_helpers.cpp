...
TEST(PearsonCorrelationTest, ZeroCorrelation) {
    ...
    EXPECT_NEAR(PearsonCorrelation(x, y), 0.0, 1e-12);
}
...
TEST(PearsonCorrelationTest, KnownValues) {
    ...
    EXPECT_NEAR(PearsonCorrelation(x, y), 0.7745966692414834, 1e-10);
}

TEST(PearsonCorrelationTest, SelfCorrelation) {
    std::vector<double> x = {3.14, 2.72, 1.41, 1.62, 0.58};
    EXPECT_NEAR(PearsonCorrelation(x, x), 1.0, 1e-12);
}
...
TEST(PearsonCorrelationTest, ScaleInvariant) {
    ...
    EXPECT_NEAR(PearsonCorrelation(x, y),
                PearsonCorrelation(x_scaled, y), 1e-10);
}
...
TEST(FibonacciSphereTest, CenterOffsetApplied) {
    ...
    double r = 1.0;
    auto pts = FibonacciSpherePoints(cx, cy, cz, r, 50);
    for (const auto& p : pts) {
        double dx = p[0] - cx;
        double dy = p[1] - cy;
        double dz = p[2] - cz;
...
TEST(FibonacciSphereTest, SinglePointIsNorthPole) {
    auto pts = FibonacciSpherePoints(5.0, 10.0, 15.0, 3.0, 1);
...
TEST(FibonacciSphereTest, PointsAreDistributed) {
    ...
    auto pts = FibonacciSpherePoints(0, 0, 0, 1.0, 1000);
...
TEST(FibonacciSphereTest, FirstAndLastPointsArePoles) {
    auto pts = FibonacciSpherePoints(0, 0, 0, 1.0, 50);
...
TEST(FibonacciSphereTest, ConsistencyWithPython) {
    ...
    auto pts = FibonacciSpherePoints(0, 0, 0, 0.5, 8);
...
TEST(EDIAmSigmoidTest, AtOneReturnsHalf) {
    EXPECT_NEAR(EDIAmSigmoid(1.0), 0.5, 1e-12);
}

TEST(EDIAmSigmoidTest, MonotonicallyIncreasing) {
    double prev = EDIAmSigmoid(-10.0);
    for (double x = -9.0; x <= 10.0; x += 0.5) {
        double val = EDIAmSigmoid(x);
...
TEST(ScoringRadiusTest, LowResolutionClamped) {
    ...
    double r = ScoringRadius(20.0, 0.5);
    double r_clamped = ScoringRadius(20.0, 1.0);
    EXPECT_NEAR(r, r_clamped, 1e-10);
}

TEST(ScoringRadiusTest, MinimumFloor) {
    // Result should never be below 0.7
    double r = ScoringRadius(1.0, 1.0);
    EXPECT_GE(r, 0.7);
}

TEST(ScoringRadiusTest, IncreasingWithBFactor) {
    double r_low = ScoringRadius(10.0, 2.0);
    double r_high = ScoringRadius(50.0, 2.0);
...
TEST(MapStatsTest, EmptyReturnsZeros) {
    auto stats = ComputeMapStats(static_cast<const double*>(nullptr), 0);
...
TEST(MapNormalizationTest, KnownValues) {
    ...
    double A, B;
    GetMapNormalization(v.data(), v.size(), A, B);
