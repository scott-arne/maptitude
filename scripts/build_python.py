#!/usr/bin/env python3
"""
Build and package maptitude for PyPI distribution.

This script builds:
1. maptitude: Binary wheel with compiled extensions
...
def get_lib_version(project_dir):
    """Get the maptitude version from pyproject.toml."""
...
def build_maptitude(project_dir, python_exe, openeye_root, openeye_info, verbose=False):
    """Build the maptitude binary wheel."""
    print_header("Building maptitude (binary wheel)")
    ...
    cmd = [
        ...
        '-C', 'cmake.define.MAPTITUDE_BUILD_TESTS=OFF',
        '-C', 'logging.level=INFO',
    ]
    run_command(cmd, cwd=project_dir, verbose=verbose)

    # Find the built wheel
    wheels = list(Path(project_dir, 'dist').glob('maptitude-*.whl'))
    if not wheels:
    ...
    print_success(f"maptitude wheel: {wheel_file}")
    return wheel_file
...
def fix_rpath_and_sign(wheel_file, openeye_info):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Find the .so file
        so_file = tmpdir / 'maptitude' / '_maptitude.so'
        if so_file.exists():
            # Add rpath
            try:
                subprocess.run([
                ...
                dylibs_dir = tmpdir / 'maptitude' / '.dylibs'
...
def upload_to_pypi(dist_dir, test_pypi=False, verbose=False):
    ...
    packages = list(Path(dist_dir).glob('maptitude*.whl'))
    if not packages:
...
def main():
    parser = argparse.ArgumentParser(
        description="Build and package maptitude for PyPI distribution",
    ...
    print_header("Maptitude Python Package Builder")
    ...
    if lib_version:
        print_step(f"maptitude version: {lib_version}")

    # Clean dist if requested
    dist_dir = project_dir / 'dist'
    if args.clean and dist_dir.exists():
        print_step("Cleaning dist/...")
        for f in dist_dir.glob('maptitude*'):
            f.unlink()
    ...
    if openeye_info:
        ...
        print_error("Cannot build maptitude without openeye-toolkits")
    ...
    if wheel:
        ...
        print_error("Failed to build maptitude")
        return 1
    ...
    print(f"  twine upload {dist_dir}/maptitude*")
    print("\nTo upload to TestPyPI:")
    print(f"  twine upload --repository testpypi {dist_dir}/maptitude*")
