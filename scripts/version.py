#!/usr/bin/env python3
"""Manage version numbers across the maptitude project.

Provides commands to display, bump, and sync the version consistently
across CMakeLists.txt, Python packages, and pyproject.toml files.
...
def _locations() -> list[VersionLocation]:
    ...
    return [
        ...
        VersionLocation(
            ...
            pattern=r"(project\(maptitude VERSION )\d+\.\d+\.\d+",
            replacement=r"\g<1>{major}.{minor}.{patch}",
            extract=r"project\(maptitude VERSION (\d+\.\d+\.\d+)",
        ),
        # Root pyproject.toml (scikit-build-core, for wheel builds)
        VersionLocation(
            file=PROJECT_ROOT / "pyproject.toml",
            label="pyproject.toml (wheel)",
            pattern=r'(^version\s*=\s*")[\d.]+(")',
            replacement=r'\g<1>{major}.{minor}.{patch}\g<2>',
            extract=r'^version\s*=\s*"([\d.]+)"',
        ),
        # python/pyproject.toml (setuptools, for editable installs)
        VersionLocation(
            file=PROJECT_ROOT / "python" / "pyproject.toml",
            label="pyproject.toml (dev)",
            pattern=r'(^version\s*=\s*")[\d.]+(")',
            replacement=r'\g<1>{major}.{minor}.{patch}\g<2>',
            extract=r'^version\s*=\s*"([\d.]+)"',
        ),
        ...
        VersionLocation(
            file=PROJECT_ROOT / "python" / "maptitude" / "__init__.py",
            label="__version__",
...
def _get_canonical_version() -> Optional[tuple[int, int, int]]:
    ...
    """
    loc = _locations()[1]  # Root pyproject.toml
...
@click.group()
def cli():
    """Manage maptitude version numbers across all project files."""
...
def get():
    ...
    table = Table(
        title="maptitude version numbers",
        box=box.ROUNDED,
        show_lines=True,
